...
nr_storage = 1
storage_max = nr_storage * 8.7  #Capacity in kWh
...
for bus in buses:
    ...
    grid_dict[bus] = house.new_scenario(column = 'baseload')
...
if len(buses_with_pv) > 0:
    ...
    for bus, df in grid_dict.items():
        if bus in buses_with_pv:
            ...
            grid_dict[bus] = house.pv_generation(irradiation, pv_size, df)
...
if len(buses_with_hp) > 0:
    ...
    for bus, df in grid_dict.items():
        if bus in buses_with_hp:
            ...
            grid_dict[bus] =  house.hp_loadshape(building_type, SigLinDe, 
...
if len(buses_with_bev) > 0:
    ...
    for bus, df in grid_dict.items():
        if bus in buses_with_bev:
            ...
            grid_dict[bus] = house.bev_loadshape(work_start, work_end, 
...
for bus, df in grid_dict.items():
    ...
    grid_dict[bus] = house.combine_loadshapes(df)
...
if len(buses_with_storage) > 0:
    ...
    for bus, df in grid_dict.items():
        if bus in buses_with_storage:
            ...
            grid_dict[bus] = house.el_storage(df, init_storage_charge, storage_max, charger_power, time_base)
